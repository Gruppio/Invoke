...
class KeychainWithPrefixTests: XCTestCase {
  ...
  func testSet() {
    ...
    XCTAssertEqual("prefixed", prefixed.getString(key)!)
    XCTAssertEqual("non prefixed", nonPrefixed.getString(key)!)
  }
  ...
  func testDelete() {
    ...
    XCTAssert(prefixed.getString(key) == nil)
    XCTAssertFalse(nonPrefixed.getString(key) == nil) // non-prefixed still exists
  }
