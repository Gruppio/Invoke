...
//  KeychainSwift is based on https://github.com/marketplacer/keychain-swift
//
...
public class KeychainSwift {
    ...
    /**
     ...
     Stores the string array value in the keychain item under the given key.
     
     - parameter key: Key under which the text value is stored in the keychain.
     - parameter value: String array to be written to the keychain.
     - parameter withAccess: Value that indicates when your app needs access to the text in the keychain item. By default the .AccessibleWhenUnlocked option is used that permits the data to be accessed only while the device is unlocked by the user.
    ...
    /**
     ...
     Retrieves the string array value from the keychain that corresponds to the given key.
     
     - parameter key: The key that is used to read the keychain item.
     - returns: The string array value from the keychain. Returns nil if unable to read the item.
    ...
    public func getStringArray(_ key: String) -> [String]? {
        if let data = getData(key) {
            ...
            if let currentString = NSString(data: data, encoding: String.Encoding.utf8.rawValue) as? String {
                return currentString.components(separatedBy: "|")
            }
